...
//       // Handle pack size (e.g. ABC-PK6 => qty ×6)
...
//         // ✅ If no expiryDate slots exist, pick the first non-zero or least-filled
//         if (!targetSlot) {
//           targetSlot = slotRows[0];
//           for (const slot of slotRows) {
//             if (slot.quantity > 0) {
//               targetSlot = slot;
//               break;
...
//         console.log(`🧩 Updating ${baseSku} in ${targetSlot.batchId} (+${totalQty}) [Expiry: ${targetSlot.expiryDate || "N/A"}]`);

//         // ✅ Update that one slot (row) only
//         await conn.query(
//           `
//           UPDATE inventory
...
//         // ✅ Update last updated time for SKU (no totalQty column)
//         await conn.query(
//           `
//           UPDATE sku
...
//       }
//       // ✅ COMBO SKU — distribute to child SKUs (no slot update)
//       else {
...
//           "SELECT sku_id, quantity FROM combo_skus_items WHERE combo_sku_id = ?",
//           [comboId]
//         );
...
//           // Update total quantity in inventory only (no slots for combo)
//           await conn.query(
//             `
//             UPDATE inventory 
//             SET quantity = quantity + ?, inventoryUpdatedAt = NOW()
...
//     return res.json({
//       success: true,
//       message: `Inventory updated for ${goodRtos.length} selected RTO(s)`,
//       totalUpdated: goodRtos.length,
//     });
//   } catch (err) {
...
// 2. Update RTOs Data to Master Inventory Phase 2
export const updateInventoryFromRtoMultiple = async (req, res) => {
  let conn = null;
  let isPooledConnection = false;
