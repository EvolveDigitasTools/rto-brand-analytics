...
const Flipkart = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/rto-upload?source=flipkart`, {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="amazonn-wrapper">
      <h1 className="amazonn-heading">Flipkart RTO Data Upload</h1>
      ...
      {(uploading || success || errorMsg) && (
        <div style={{ width: "95%", margin: "20px auto 0", textAlign: "left" }}>
          ...
          {success && (
            ...
            >
              ✅ Flipkart RTO upload completed successfully.
            </div>
          )}
          {errorMsg && (
            <div
